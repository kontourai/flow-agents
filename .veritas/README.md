...
This repo was bootstrapped for `flow-agents` with a conservative starter kit for agent-guided development.
...
- Repo kind: `application`
- Evidence Check: `npm run validate:source && npm run test:unit && npm run eval:static`
...
`npm run validate:source && npm run test:unit && npm run eval:static`
