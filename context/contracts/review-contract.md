...
```bash
npm run workflow:sidecar -- record-critique .agents/flow-agents/<slug> \
  --id code-review \
...
```bash
npm run workflow:sidecar -- import-critique .agents/flow-agents/<slug> \
  .agents/flow-agents/<slug>/<slug>--code-review.md \
  --reviewer tool-code-reviewer
```
