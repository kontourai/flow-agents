...
tool-worker manages its own TODO files in `.agents/flow-agents/<slug>/` and tracks `modified_files` to detect conflicts with other parallel workers. It will end its turn immediately if instructions are insufficient rather than guessing.

## Conflict Avoidance

- tool-worker checks `.agents/flow-agents/` for in-progress work from other instances
