...
| --- | --- | --- | --- | --- |
| `flow` | `flow-definition-transition-guard-impl...origin/flow-definition-transition-guard-impl`; untracked `.agents/` | 0 TS/TSX, 2 JS, 1 MJS, 0 Python. Small JS/MJS runtime and schema-check surface. | `check:schemas` and `test`; no TypeScript typecheck yet. | Track migration in [Flow #20](https://github.com/kontourai/flow/issues/20). |
...
| `kontourai.io` | `codex/survey-extraction-readiness-audit...origin/codex/survey-extraction-readiness-audit`; untracked `.agents/` | 0 TS/TSX, 1 `.d.ts`, 1 JS, 4 MJS, 0 Python. Astro/TypeScript-enabled site with environment typing, JS worker asset, MJS config/scripts/tests. | `build`, `validate`, and rendered-site test scripts are present; Astro build provides the TypeScript-enabled site validation path. | No new issue currently needed; remaining JS/MJS files fit accepted site/config/test/asset exceptions unless future product source expands in JS. |
