...
| `evals/fixtures/narrative-sources` | narrative source snapshot, offline resolution, integrity, completeness, fail-closed redaction, and grounded runtime projection fixtures (#617, #618) | `evals/integration/test_narrative_source_contract.sh`, `evals/integration/test_narrative_runtime_projection.sh`, `evals/integration/test_narrative_redaction_failclosed.sh` | Keep while the narrative source manifest, content-addressed offline resolver, integrity verification, grounded runtime projection, capture-completeness disclosure, lifecycle locality, and fail-closed redaction contract are supported. |
