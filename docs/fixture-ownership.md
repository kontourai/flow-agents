...
| `evals/fixtures/narrative-sources` | narrative source snapshot, offline resolution, integrity, completeness, and fail-closed redaction fixtures (#617) | `evals/integration/test_narrative_source_contract.sh`, `evals/integration/test_narrative_redaction_failclosed.sh` | Keep while the narrative source manifest, content-addressed offline resolver, integrity verification, capture-completeness disclosure, lifecycle locality, and fail-closed redaction contract are supported. |
