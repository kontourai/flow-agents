...
This roadmap turns the Flow Agents North Star into incremental work:
https://github.com/kontourai/flow-agents/blob/main/docs/north-star.md
...
- Multiple agents can share one workflow root by resolving `.agents/flow-agents/current.json` and appending agent-local events instead of racing on root state.
