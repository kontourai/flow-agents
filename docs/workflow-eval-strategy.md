...
Activation-only behavioral evals may assert no write tools when the goal is trigger and boundary testing. Artifact-quality evals must allow controlled writes to `.agents/flow-agents/<slug>/*.md` and inspect the resulting artifact contracts.
...
Inspect generated `.agents/flow-agents/<slug>/*.md` files and provider-backed work item drafts for required structure.
...
- Agent tries to stop with an active `.agents/flow-agents/<slug>/` delivery artifact; `stop-goal-fit` should warn and strict mode should block.
