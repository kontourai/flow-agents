...
fixture_group_count="$(find "$ROOT/evals/fixtures" -mindepth 1 -maxdepth 1 -type d | wc -l | tr -d ' ')"
[[ "$(json_query "$TMPDIR_EVAL/audit.json" "totals.scanned")" == "$fixture_group_count" ]] && pass "audit scans all fixture groups" || fail "audit scans all fixture groups"
...
[[ "$(json_query "$TMPDIR_EVAL/audit.json" "totals.kept")" == "$fixture_group_count" ]] && pass "audit keeps all owned fixture groups" || fail "audit keeps all owned fixture groups"
