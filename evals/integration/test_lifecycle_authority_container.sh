...
# The privileged coordinator is pinned to the audited Flow 3.11.0 reducer closure, and that pin is
...
await flow.pauseRun(runId, {
  cwd: project,
  reason: 'installed Flow 3.11.0 mutation after reseal',
  authority: {
    kind: 'operator_request',
    actor: 'installed-flow-3.11.0-container',
...
if (packageJson.name !== '@kontourai/flow' || packageJson.version !== '3.11.0') throw new Error('reseal interoperability must use installed exact @kontourai/flow@3.11.0');
...
if (['withRunRecoveryLock', 'writeRunRecoveryFence', 'finalizeRunRecoveryFence'].some((name) => typeof installedStore[name] !== 'function')) {
  throw new Error('installed exact @kontourai/flow@3.11.0 does not expose the public recovery contract');
}
...
await Promise.race([
  ...
  holder.result.then((result) => { throw new Error(`installed 3.11.0 holder exited before readiness: ${result.stdout}${result.stderr}`); }),
]);
const reseal = spawnAsNode('/work/reseal-invoke.mjs', [project, session, '/root/lifecycle-authorizations/reseal.json']);
await Promise.race([
  waitFor('root reseal ticket behind installed 3.11.0 holder', () => ticketCount() === 2),
  reseal.result.then((result) => { throw new Error(`root reseal exited before queuing behind installed 3.11.0: ${result.stdout}${result.stderr}`); }),
]);
const pause = spawnAsNode('/work/pinned-flow-pause.mjs', [pinnedFlowRoot, project, runId]);
await Promise.race([
  waitFor('installed 3.11.0 public pause behind reseal', () => ticketCount() === 3),
  pause.result.then((result) => { throw new Error(`installed 3.11.0 pause exited before queuing: ${result.stdout}${result.stderr}`); }),
]);
fs.writeFileSync(releaseFile, 'release\n');
const [holderResult, resealResult, pauseResult] = await Promise.all([holder.result, reseal.result, pause.result]);
if (holderResult.status !== 0) throw new Error(`installed 3.11.0 holder failed: ${holderResult.stderr}`);
if (resealResult.status !== 0) throw new Error(`root reseal failed behind installed 3.11.0 lock: ${resealResult.stdout}${resealResult.stderr}`);
if (pauseResult.status !== 0) throw new Error(`installed 3.11.0 pause failed behind reseal: ${pauseResult.stderr}`);
