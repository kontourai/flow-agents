...
node --test \
  --test-name-pattern='configured ChangeProvider projects|Flow completion authenticates|publish-change rejects symlinked|stale publish-change actions|same publish-change action|provider failures cannot leak|public publish-change execute composes' \
