...
if flow_agents_node "$WRITER" record-check "$SESSION_DIR" \
  ...
  && node - "$SESSION_DIR/trust.bundle" <<'NODE'
...
if (claim.metadata.recorded_by !== 'record-check-verifier') {
  process.stderr.write('metadata.recorded_by=' + JSON.stringify(claim.metadata.recorded_by) + ' (expected "record-check-verifier") — the record-check actor stamp was deleted, or moved above the normalizeCheck call that scrubs it\n');
  process.exit(2);
}
const event = bundle.events.find((e) => e.claimId === claim.id);
if (event?.actor !== 'record-check-verifier') {
  process.stderr.write('events[].actor=' + JSON.stringify(event && event.actor) + ' (expected "record-check-verifier")\n');
  process.exit(3);
}
...
if (evidence.length === 0 || !evidence.every((e) => e.collectedBy === 'record-check-verifier')) {
  process.stderr.write('evidence collectedBy=' + JSON.stringify(evidence.map((e) => e.collectedBy)) + ' (expected all "record-check-verifier")\n');
  process.exit(4);
}
if (!evidence.every((e) => e.supportStrength === 'entails')) {
  process.stderr.write('supportStrength=' + JSON.stringify(evidence.map((e) => e.supportStrength)) + ' (expected all "entails" — a writer-executed observation with a named collector)\n');
  process.exit(5);
}
...
NODE
then
  _pass "#1363: record-check publishes the actor it resolved as the event actor and evidence collector, and its writer-executed observation entails the claim"
else
  _fail "#1363: record-check did not publish its resolved actor into the delivered bundle: $(cat "$TMPDIR_EVAL/rc-actor.err")"
fi
...
then
  ...
  _fail "#1363: same-id supersession by attestation mishandled the verifier: $(cat "$TMPDIR_EVAL/rc-supersede.err")"
fi
