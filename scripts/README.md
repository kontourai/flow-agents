...
| `subject-binding.js` | policy hook | `src/cli/subject-binding-hook.test.mjs`, `evals/integration/test_subject_collision_detection.sh` | PreToolUse first-write interception: when a file is about to be written and no Flow Agents subject is bound for this actor, names the binding command (advisory by default via `FLOW_AGENTS_SUBJECT_BINDING=warn`; `block` is opt-in). Also carries the pre-file backlog duplicate-check reminder, since the subject id IS the backlog identifier (#1099). |
...
| `lib/subject-binding.js` | shared hook library | `src/cli/subject-binding-hook.test.mjs` | Shared "is a subject bound?" primitive (`boundSubject`, `subjectStatus`, `resolveSubjectBindingMode`) consumed by both the PreToolUse binding gate and `workflow-steering.js`, so the gate and the steering can never disagree about whether a subject exists (#1099). |
| `lib/subject-identity.js` | shared hook library | `src/cli/subject-identity.test.mjs`, `evals/integration/test_subject_collision_detection.sh` | The ONE deterministic subject-identity derivation (`workItemSlugResult`, `canonicalSubjectKey`, `backlogRefsInText`): the collision key is derived from the backlog item rather than chosen, and `src/lib/work-item-identity.ts` delegates here so the hooks and the CLI cannot drift (#1099). |
