...
| `continuation-turn-fence.js` | policy hook | `src/cli/continuation-turn-fence.test.mjs`, `src/cli/continuation-driver.test.mjs` | Returns a signed one-step continuation turn to its driver immediately after canonical Flow advances, before the model starts the next step in the same turn. |
