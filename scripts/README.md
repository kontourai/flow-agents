...
| `lib/policy-record.js` | shared hook library | `evals/integration/test_policy_record_relay.sh` | Builds and durably appends `kontour.console.policy` decision records (`buildPolicyRecord`, `writePolicyRecord`), emitted from the runtime-neutral `run-hook.js` so every runtime is covered by one implementation (issue #1025). |
| `lib/record-relay.js` | shared hook library | `evals/integration/test_policy_record_relay.sh`, `evals/integration/test_liveness_console_relay.sh` | One family-parameterized console relay primitive (`resolveRelayEnabled`, `relayRecord`) — conf-key > env > default-on-once-configured, local-first and detached. Replaces the per-family copies liveness and economics each carried (issue #1025). |
