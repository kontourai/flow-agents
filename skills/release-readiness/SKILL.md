...
Create or update `.agents/flow-agents/<slug>/<slug>--release-readiness.md` with:
...
- `final_acceptance_docs`: long-lived docs updated, archived `.agents/flow-agents/<slug>/` links, and deferred docs follow-ups
...
```bash
npm run workflow:sidecar -- record-release .agents/flow-agents/<slug> \
  --decision merge \
  --scope "..." \
...
- link back to the archived `.agents/flow-agents/<slug>/` plan/session artifact for implementation history.
