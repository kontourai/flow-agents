...
import { buildWorkflowProcessProjection, readWorkflowProcessSources } from "../lib/workflow-process-projection.js";
...
function printHelp(): void {
  ...
  console.log("");
  console.log("Build an inert Console process-state projection from local workflow state/handoff/critique sidecars (issue #778).");
  console.log("");
  console.log("Options:");
...
export function main(argv = process.argv.slice(2)): number {
  ...
  try {
    ...
    const sources = readWorkflowProcessSources(artifactRoot);
    const projection = buildWorkflowProcessProjection(sources, {
      scope,
    ...
    const summary: Summary = {
      scanned_state_file_count: sources.length,
