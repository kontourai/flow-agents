...
function effective(repoPath: string, projectSettings: string, globalSettings: string): [Record<string, unknown>, number] {
  ...
  return [{ status: "configured", current_repo: repo, source: projectProject || projectDoc?.defaults ? "project" : "global", precedence: ["project.projects path match", "project.defaults", "global.projects path match", "global.defaults"], settings: effectiveSettings }, 0];
}
