...
function action(overrides = {}) {
  return issueMergeChangeAction({ binding, provider, assignment_actor: "codex:1000:Kontour", intent: { strategy: "squash", change_number: 1000, base_ref: "main", head_ref: "fix/terminal-before-merge-1000", terminal_head_sha: SHA, ...overrides } });
}
...
test("canonical completed-run evidence authenticates the publish action identity and exact result digest", () => {
  ...
  const observation = {
    ...
    provider: { kind: "github", configuration_id: issueMergeChangeAction({ binding, provider, assignment_actor: "codex:1000:Kontour", intent: { strategy: "squash", change_number: 1000, base_ref: "main", head_ref: "fix/terminal-before-merge-1000", terminal_head_sha: SHA } }).provider.configuration_id, adapter: "github-gh-cli" },
