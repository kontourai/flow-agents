...
function readPidState(pid) {
  const result = spawnSync("ps", ["-o", "pid=,ppid=,pgid=,stat=,lstart=", "-p", String(pid)], { encoding: "utf8" });
...
test("sealed timeout kills the detached process group including a provider descendant", async () => {
  await withFixtureCaller(async () => {
    ...
    try {
      ...
      assert.ok(descendantIdentity, `timeout fixture descendant is not a readable owned PID: ${descendant}`);
