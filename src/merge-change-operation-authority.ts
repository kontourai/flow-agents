...
export function issueMergeChangeAction(input: Readonly<{ binding: PublishChangeActionBinding; provider: ChangeProviderSettings; assignment_actor: string; intent: MergeChangeIntent }>): IssuedMergeChangeAction {
  const request = {
    ...
    intent: intent(input.intent), assignment_actor: bounded(input.assignment_actor, "assignment_actor"),
...
export function assertIssuedMergeChangeAction(value: unknown): IssuedMergeChangeAction {
  ...
  const entry = value as Record<string, unknown>;
  const keys = ["schema_version", "operation", "binding", "repository", "intent", "assignment_actor", "provider", "action_id"];
  ...
  const request = { schema_version: "1.0" as const, operation: "merge-change" as const, binding: binding(entry.binding), repository: repository(entry.repository), intent: intent(entry.intent), assignment_actor: bounded(entry.assignment_actor, "action.assignment_actor"), provider: { kind: "github" as const, configuration_id: bounded(provider.configuration_id, "action.provider.configuration_id", 64) } };
...
export function assertAuthenticatedMergeChangeObservation(actionInput: IssuedMergeChangeAction, value: unknown): AuthenticatedMergeChangeObservation {
  ...
  return Object.freeze({ schema_version: "1.0" as const, operation: "merge-change" as const, binding: structuredClone(action.binding), repository: structuredClone(action.repository), intent: structuredClone(action.intent), provider: { kind: "github" as const, configuration_id: action.provider.configuration_id, adapter: "github-gh-cli" as const }, assignment_actor: action.assignment_actor, provider_actor: bounded(entry.provider_actor, "provider observation.provider_actor"), state, ...(merge_sha ? { merge_sha } : {}), ...(queue_entry ? { queue_entry } : {}), observed_at });
}
