...
export function decodeRetainedNarrativeProcessProjection(value: unknown): RetainedNarrativeProcessProjection | undefined {
  ...
  for (let index = 0; index < inputTurns.length; index += 1) {
    ...
    turns.push({ ordinal, boundary: boundary.rule_id === undefined ? { derived: boundary.derived } : { derived: boundary.derived, rule_id: "turn-spine/v1" }, actions });
  }
  ...
  return {
    ...
    capture: { channels: { active: channels.active as number, inactive: channels.inactive as number, unknown: channels.unknown as number }, knownGapClasses },
